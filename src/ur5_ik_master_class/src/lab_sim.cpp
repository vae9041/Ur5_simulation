...
static constexpr double kGraspZOffset = 0.08;
static constexpr double kNearPickLift = 0.05;
...
public:
    ...
    void close_gripper()
    {
        ...
        gripper_.setJointValueTarget("robotiq_85_left_knuckle_joint", 0.4);  // Close fully using action command value
        gripper_.move();
    }
    ...
    bool pick()
    {
        ...
        geometry_msgs::msg::Pose near_pick_position = pick_position;
        near_pick_position.position.z += kNearPickLift;
        ...
        if (!planAndExecutePose(near_pick_position, "tool0", "near-pick")) {
            RCLCPP_ERROR(logger_, "Failed to reach near-pick pose. Aborting pick.");
            return false;
        }
        ...
        // ----------------------------
        if (!executeCartesianDown(near_pick_position, pick_position)) {
